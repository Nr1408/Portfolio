...
function createConfigurationReader() {
	return async (configurationPath, options = { configName: defaultConfigName }) => {
		...
		try {
			...
			const { content, collections, hooks,...rest } = (await import(`file://${path.resolve(outfile)}?x=${Date.now()}`)).default;
...
async function createDataFile(directory, collection, hooks) {
	...
	let content = serialize(isSingleton(collection) ? collection.documents[0]?.document : collection.documents.map((doc) => doc.document));
	for (const hook of hooks) content = (await hook({
		fileType: "data",
		content,
		filePath: dataPath
	})).content;
...
function createDataFiles(directory, collections, hooks) {
	return Promise.all(collections.map((collection) => createDataFile(directory, collection, hooks)));
}
async function createJavaScriptFile(directory, configuration, hooks) {
...
async function createTypeDefinitionFile(directory, configuration, hooks) {
...
async function createWriter(directory, hooks) {
	await fs.mkdir(directory, { recursive: true });
	return {
		createJavaScriptFile: (configuration) => createJavaScriptFile(directory, configuration, hooks),
		createTypeDefinitionFile: (configuration) => createTypeDefinitionFile(directory, configuration, hooks),
		createDataFiles: (collections) => createDataFiles(directory, collections, hooks)
	};
}
...
async function createWatcher(emitter, baseDirectory, configuration, sync) {
	...
	function isInsideWatchPath(filePath, watchPath) {
		const relativePath = path.relative(watchPath, filePath);
		return relativePath !== "" && !relativePath.startsWith("..") && !path.isAbsolute(relativePath);
	}
	function isHiddenInWatchedTree(filePath) {
		return paths.some((watchPath) => {
			if (!isInsideWatchPath(filePath, watchPath)) return false;
			const relativePath = path.relative(watchPath, filePath);
	...
	const watcher = chokidar.watch(paths, {
		ignored: (filePath) => {
			...
			if (/(^|[\/\\])node_modules([\/\\]|$)/.test(absolutePath)) return true;
